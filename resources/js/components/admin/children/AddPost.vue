      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <div v-if="post.image && !previewImage" class="mb-2">
            <v-img :src="post.image_path_aws" alt="image" v-if="edit"> </v-img>
          </div>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="hideAddPostDialog()">
            Close
          </v-btn>
...
export default {
  ...
  methods: {
    fetchCategories() {
      axios
        .get("/api/categories", this.config)
    ...
    addPost() {
      ...
      axios
        .post("/api/posts", this.form, this.config)
        .then((res) => {
    ...
    updatePost() {
      ...
      axios
        .post("/api/posts/" + this.showPost.id, form, this.config)
        .then((res) => {
          this.previewImage = "";
