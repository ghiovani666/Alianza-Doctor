<template>
  <div>
    <v-card class="mx-auto" width="600px">
      ...
      <v-data-table :headers="headers" :items-per-page="5" :items="categories">
       <template v-slot:[`item.actions`]="{ item }">
          <v-icon
            color="primary"
            small
            class="mr-2"
            @click.stop="(dialog = true), (edit = true), editCategory(item.id)"
          >
            mdi-pencil
          </v-icon>
          <v-icon
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Numero de Post", value: "posts.length" },
  ...
  methods: {
    ...
    editCategory(id) {
      axios
        .get("/api/categories/" + id, this.config)
